...
void schedule_execute::add_process(process_details &process,ofstream &file)
{
	...
	else
	{
		...
		for(auto it = process_queue.begin() ; it != process_queue.end() ; it++,i++)
		{
			...
			//check the periods of coming process and process in the queue
			if(process.period < it->period)
			{
			...
			//if the periods are equal, then check the process id and assign its place in processing queue
			else if(process.period == it->period)
			{
...
{
	int sum = 0,it_flag = 0,process_flag = 0;
	...
	{
		//it flag is for getting the respective period time at an instance
		it_flag = 1 + (it->repeat_count - it->number_of_times);
		//condition for checking if a process misses it's deadline
		if(real_time + it->remaining_time > it_flag*it->period)
		break;
	}
...
void schedule_execute::execute(vector<process_details> &details,ofstream &file)
{
	...
	if(flag1 or flag2)
	{
		...
		file<<"process p"<<process.process_id<<" has resumed it's execution"<<endl;
	}
...
{
	double tempvar1 = 0,tempvar2 = 0;
	for(int i = 0 ; i < number_of_processes ; i++)
	{
		tempvar1 = tempvar1 + double(details[i].processing_time)/(details[i].period);
	}
	tempvar2 = (number_of_processes * (pow(2, ((1/(double)number_of_processes))) - 1));
...
void rate_monotonic_scheduler(vector<process_details> &details,int number_of_processes)
{
	ofstream outlogfile("RMS-log.txt");
...
void print_stats(vector<process_details> &details,int number_of_processes)
{
	...
	ofstream outstatsfile("RMS-stats.txt");
...
int main()
{
	...
	if(inputfile)
	{
		...
		rate_monotonic_scheduler(processes,number_of_processes);
